...
t.data=tg.call(e,e.transformResponse,t),t.headers=th.from(t.headers),t},function(t){return!tb(t)&&(tL(e),t&&t.response&&(t.response.data=tg.call(e,e.transformResponse,t.response),t.response.headers=th.from(t.response.headers))),Promise.reject(t)})}const tM=e=>e instanceof th?e.toJSON():e;function tz(e,t){// eslint-disable-next-line no-param-reassign
...
const il={allCategiries:document.querySelector(".categories-btn-all-js"),categories:document.querySelector(".categories-js"),popular:document.querySelector(".popular-change-js"),paginationShow:document.querySelector("#tui-pagination-container"),pagination:document.getElementById("tui-pagination-container")};// налаштування початкових значень в залежносты від ширини вьюпорту
let ic=window.innerWidth<768?6:window.innerWidth<1280?8:9,ip=1,id=window.innerWidth<768?2:3;// екземпляр класу з параметрами для запиту рецептів по філтру
const iu=new URLSearchParams({page:ip,limit:ic}),im={totalItems:0,itemsPerPage:ic,visiblePages:id,page:ip},ih=new/*@__PURE__*/(f(io))(il.pagination,im);// перемальовка блоку рецептів після зміни параметрів запиту з 1 сторынки з перемальовкою пагінації
...
let t=il.popular.querySelectorAll(".popular-button-js");t.forEach(e=>{e.addEventListener("click",iw)})}).catch(()=>{});const iE=iS();function iC(e){let t=/**
  |============================
  | Запит для Swiper
  |============================
*//**
