...
let{Axios:tD,AxiosError:tj,CanceledError:tF,isCancel:tW,CancelToken:t$,VERSION:tU,all:tH,Cancel:tX,isAxiosError:tG,spread:tq,toFormData:tV,AxiosHeaders:tY,HttpStatusCode:tK,formToJSON:tJ,getAdapter:tQ,mergeConfig:tZ}=tR,t0=document.querySelector(".card-list"),t1=async(e,t,i,n,r,a)=>{// Праметри API запиту
...
r(t,function(e,i){0===e.indexOf("if")?o+=1:"/if"===e?o-=1:o||0!==e.indexOf("elseif")&&"else"!==e||(n.push("else"===e?["true"]:e.split(" ").slice(1)),a.push(t.slice(s,i)),s=i+1)}),a.push(t.slice(s)),{exps:n,sourcesInsideIf:a}),c=!1,p="";return r(l.exps,function(e,t){return(c=x(e,i))&&(p=v(l.sourcesInsideIf[t],i)),!c}),p},each:/**
 ...
 * @private
 */// eslint-disable-next-line complexity
function y(e,t){var i,n=t[e];return"true"===e?n=!0:"false"===e?n=!1:u.test(e)?n=e.replace(m,""):c.test(e)?n=y((i=e.split(p))[0],t)[y(i[1],t)]:d.test(e)?n=y((i=e.split(f))[0],t)[i[1]]:h.test(e)&&(n=parseFloat(e)),n}/**
 ...
 * @private
 */function(e,t,i){for(var n,r,a,s=g[e],l=1,c=2,p=t[2];l&&o(p);)0===p.indexOf(e)?l+=1:0===p.indexOf("/"+e)&&(l-=1,a=c),c+=2,p=t[c];if(l)throw Error(e+" needs {{/"+e+"}} expression.");return t[0]=s(t[0].split(" ").slice(1),(n=a,(r=t.splice(1,n-0)).pop(),r),i),t}(n,e.splice(a,e.length-a),t),e=e.concat(r)):e[a]=x(i,t),a+=2,s=e[a];return e.join("")}e.exports=/**
...
let is={allCategiries:document.querySelector(".categories-btn-all-js"),categories:document.querySelector(".categories-js"),popular:document.querySelector(".popular-change-js"),paginationShow:document.querySelector("#tui-pagination-container"),pagination:document.getElementById("tui-pagination-container")},il=window.innerWidth<768?6:window.innerWidth<1280?8:9,ic=1,ip=window.innerWidth<768?2:3,id=new URLSearchParams({page:ic,limit:il}),iu={totalItems:0,itemsPerPage:il,visiblePages:ip,page:ic},im=new/*@__PURE__*/(f(io))(is.pagination,iu);// перемальовка блоку рецептів після зміни параметрів запиту з 1 сторынки з перемальовкою пагінації
function ih(){im.off("afterMove",ig),iw("page",1),ir(id).then(e=>{t2(e),// встановлення слухача пагінації, відображення її
function(e){if(im.reset(e*il),e>1){im.on("afterMove",ig),is.paginationShow.classList.remove("is-hidden");return}is.paginationShow.classList.contains("is-hidden")||is.paginationShow.classList.add("is-hidden"),e||/*@__PURE__*/f(ia).Notify.info("Вибачте, згідно параметрів пошуку рецептів не знайдено")}(e.data.totalPages)}).catch(()=>{})}// перемальовка рецептів при зміщенні пагінації
function ig(){iw("page",ic=im.getCurrentPage()),ir(id).then(e=>{t2(e)}).catch(()=>{})}// вибір "всіх категорій" по натисканню "AllCategories"
function ib(){ix(),is.allCategiries.classList.add("is-active"),ik("category"),ih()}// вибір конкретної категорії
function iy(e){if("BUTTON"!==e.target.nodeName)return;is.allCategiries.classList.contains("is-active")&&is.allCategiries.classList.remove("is-active"),ix(),e.target.classList.add("is-active");let t=e.target.dataset.set;iw("category",t),ih()}// знімаємо помітку з активної категорії
function ix(){let e=is.categories.querySelector(".is-active");e&&e.classList.remove("is-active")}// обробка вибору популярного рецепту
function iv(e){if("BUTTON"!==e.currentTarget.nodeName)return;let t=e.currentTarget.dataset.set;console.log(t)}// додавання/зміна параметру в об'єкт для пошуку рецепту
function iw(e,t){id.has(e)&&id.delete(e),id.append(e,t)}// вилучення параметра в об'єкті пошуку рецепту
function ik(e){for(;id.has(e);)id.delete(e)}/**
  |============================
  | Refs hero
  |============================
...
is.allCategiries.addEventListener("click",ib),is.categories.addEventListener("click",iy),is.popular.addEventListener("click",iv),// прибирання слухачів подій
window.addEventListener("unload",function(){is.allCategiries.removeEventListener("click",ib),is.categories.removeEventListener("click",iy),is.popular.removeEventListener("click",iv),im.off("afterMove",ig)}),t9().then(e=>{is.categories.insertAdjacentHTML("beforeend",// формування розмітки переліку категорій
...
ih(),ie().then(e=>{is.popular.insertAdjacentHTML("beforeend",e.data.map(e=>`
...
let t=is.popular.querySelectorAll(".popular-button-js");t.forEach(e=>{e.addEventListener("click",iv)})}).catch(()=>{});let iE=iS();function iC(e){let t=/**
  |============================
  | Запит для Swiper
  |============================
*//**
...
if(l.normalizeSlideIndex)for(let e=0;e<p.length;e+=1){let t=-Math.floor(100*y),i=Math.floor(100*p[e]),n=Math.floor(100*p[e+1]);void 0!==p[e+1]?t>=i&&t<n-(n-i)/2?s=e:t>=i&&t<n&&(s=e+1):t>=i&&(s=e)}// Directions locks
...
.replace(/ /g,".")}`}Object.keys(i8).forEach(e=>{Object.keys(i8[e]).forEach(t=>{i7.prototype[t]=i8[e][t]})}),i7.use([function(e){let{swiper:t,on:i,emit:n}=e,r=iI(),a=null,o=null,s=()=>{t&&!t.destroyed&&t.initialized&&(n("beforeResize"),n("resize"))},l=()=>{t&&!t.destroyed&&t.initialized&&(a=new ResizeObserver(e=>{o=r.requestAnimationFrame(()=>{let{width:i,height:n}=t,r=i,a=n;e.forEach(e=>{let{contentBoxSize:i,contentRect:n,target:o}=e;o&&o!==t.el||(r=n?n.width:(i[0]||i).inlineSize,a=n?n.height:(i[0]||i).blockSize)}),(r!==i||a!==n)&&s()})})).observe(t.el)},c=()=>{o&&r.cancelAnimationFrame(o),a&&a.unobserve&&t.el&&(a.unobserve(t.el),a=null)},p=()=>{t&&!t.destroyed&&t.initialized&&n("orientationchange")};i("init",()=>{if(t.params.resizeObserver&&void 0!==r.ResizeObserver){l();return}r.addEventListener("resize",s),r.addEventListener("orientationchange",p)}),i("destroy",()=>{c(),r.removeEventListener("resize",s),r.removeEventListener("orientationchange",p)})},function(e){let{swiper:t,extendParams:i,on:n,emit:r}=e,a=[],o=iI(),s=function(e,i){void 0===i&&(i={});let n=o.MutationObserver||o.WebkitMutationObserver,s=new n(e=>{// The observerUpdate event should only be triggered
