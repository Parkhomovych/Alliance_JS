...
});const ic=document.querySelector(".card-list");t=375>=window.outerWidth?6:768>=window.outerWidth?8:9;const ip=async(e,i,n,o,a,r)=>{// Праметри API запиту
let s=new URLSearchParams({limit:t}),l=await eM.get(`https://tasty-treats-backend.p.goit.global/api/recipes?${s}`);return l};function id(t){ic.innerHTML=t.data.results.map(t=>`
    <li class="card-item">
...
function iC(){iS.off("afterMove",iE),iM("page",1),e2(iw).then(t=>{id(t),// встановлення слухача пагінації, відображення її
...
function iE(){iM("page",ib=iS.getCurrentPage()),e2(iw).then(t=>{id(t)}).catch(()=>{})}// вибір "всіх категорій" по натисканню "AllCategories"
...
c.params=iH({},p,ns,e),c.originalParams=iH({},c.params),c.passedParams=iH({},e),c.params&&c.params.on&&Object.keys(c.params.on).forEach(t=>{c.on(t,c.params.on[t])}),c.params&&c.params.onAny&&c.onAny(c.params.onAny),// Extend Swiper
...
nL.resetBtn.addEventListener("click",function(t){iI("time"),iI("area"),iI("ingredient"),iI("title"),iC()}//# sourceMappingURL=index.644d9088.js.map
);
//# sourceMappingURL=index.644d9088.js.map
