...
const ia=(t,e)=>{try{let i=JSON.stringify(e);localStorage.setItem(t,i)}catch(t){// обробка помилки при збереженні даних
...
let ir=[];// Завантажуємо збережені дані з локального сховища.
const is=(t=>{try{let e=localStorage.getItem(t);return null===e?void 0:JSON.parse(e)}catch(t){// Обробка помилки при завантаженні даних
console.error("Get state error: ",t.message),Notify.failure("Something went wrong. Please try again")}})("cardData");is&&(ir=is);const il=document.querySelector(".card-list");t=375>=window.outerWidth?6:768>=window.outerWidth?8:9;const ic=async(e,i,n,o,a,r)=>{// Праметри API запиту
...
function(t){let e=ir.findIndex(({_id:e})=>e===t);e<0&&eQ(t).then(({data:t})=>(t.favorit=!0,ir.push(t),ir)).then(t=>{ia("cardData",t)}).catch(()=>{console.log(error)})}(t):ir[e].favorit?ir[e].favorit=!1:ir[e].favorit=!0}(e),console.log(e))}});const ip=new MutationObserver(function(){let t=document.querySelectorAll(".info-btn");// витягуємо всі кнопки
...
const iv=new URLSearchParams({page:ix,limit:iy}),iw={totalItems:0,itemsPerPage:iy,visiblePages:ib,page:ix},ik=new/*@__PURE__*/(h(ih))(ig.pagination,iw);// перемальовка блоку рецептів після зміни параметрів запиту з 1 сторынки з перемальовкою пагінації
...
let e=ig.popular.querySelectorAll(".popular-button-js");e.forEach(t=>{t.addEventListener("click",iL)})}).catch(()=>{});const iz=iM();function iO(t){let e=/**
  |============================
  | Розмітка Swiper
  |============================
...
nT.resetBtn.addEventListener("click",function(t){iI("time"),iI("area"),iI("ingredient"),iI("title"),iS()}//# sourceMappingURL=index.d3a50faf.js.map
);
//# sourceMappingURL=index.d3a50faf.js.map
