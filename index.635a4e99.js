...
v.addEventListener("click",y);const b=document.querySelector(".switcher-check"),x=document.querySelector(".mob-switcher-check"),w=document.querySelector("body"),S={DARK:"dark-theme"};b.addEventListener("change",()=>{let e=w.classList.contains(S.DARK);x.checked=b.checked,e?w.classList.remove(S.DARK):w.classList.add(S.DARK),localStorage.setItem("theme",e?S.DARK:S.LIGHT)}),// ---------------Mob-Switcher------------------------------
x.addEventListener("change",()=>{let e=w.classList.contains(S.DARK);b.checked=x.checked,e?w.classList.remove(S.DARK):w.classList.add(S.DARK),localStorage.setItem("theme",e?S.DARK:S.LIGHT)});var u=c("goQqF"),T=c("5uEKZ");const E=document.querySelector(".card-list"),C=document.querySelector("#card-list");function k(e){E.innerHTML=((0,T.resizeFavorit)(),e.map(e=>`
    <li class="card-item">
 ...
 *     dh: 'dh'
 * });
 */function(e,t){var i=document.createElement("img"),n="";return s(t,function(e,t){n+="&"+t+"="+e}),n=n.substring(1),i.src=e+"?"+n,i.style.display="none",document.body.appendChild(i),document.body.removeChild(i),i};/***/}]);var _=c("7Y9D8"),u=c("goQqF");// посилання на Dom
const P={allCategiries:document.querySelector(".categories-btn-all-js"),categories:document.querySelector(".categories-js"),popular:document.querySelector(".popular-change-js"),paginationShow:document.querySelector("#tui-pagination-container"),pagination:document.getElementById("tui-pagination-container")};// налаштування початкових значень в залежносты від ширини вьюпорту
let I=window.innerWidth<768?6:window.innerWidth<1280?8:9,N=1,A=window.innerWidth<768?2:3;// екземпляр класу з параметрами для запиту рецептів по філтру
const O=new URLSearchParams({page:N,limit:I}),z={totalItems:0,itemsPerPage:I,visiblePages:A,page:N},B=new/*@__PURE__*/(r(L))(P.pagination,z);// перемальовка блоку рецептів після зміни параметрів запиту з 1 сторынки з перемальовкою пагінації
...
function j(){G(),P.allCategiries.classList.add("is-active"),q("category"),$()}// вибір конкретної категорії
function F(e){if("BUTTON"!==e.target.nodeName)return;P.allCategiries.classList.contains("is-active")&&P.allCategiries.classList.remove("is-active"),G(),e.target.classList.add("is-active");let t=e.target.dataset.set;W("category",t),$()}// знімаємо помітку з активної категорії
function G(){let e=P.categories.querySelector(".is-active");e&&e.classList.remove("is-active")}// обробка вибору популярного рецепту
...
function q(e){for(;O.has(e);)O.delete(e)}// слухачі подій
P.allCategiries.addEventListener("click",j),P.categories.addEventListener("click",F),P.popular.addEventListener("click",H),// прибирання слухачів подій
window.addEventListener("unload",function(){P.allCategiries.removeEventListener("click",j),P.categories.removeEventListener("click",F),P.popular.removeEventListener("click",H),B.off("afterMove",D)}),// формування переліку категорій
...
let t=P.popular.querySelectorAll(".popular-button-js");t.forEach(e=>{e.addEventListener("click",H)})}).catch(()=>{});var u=c("goQqF"),V=c("fAYBR");const R=(0,V.default)();function Y(e){let t=/**
  |============================
  | Розмітка Swiper
  |============================
 ...
 *//* eslint-disable no-param-reassign */function X(e){return null!==e&&"object"==typeof e&&"constructor"in e&&e.constructor===Object}function U(e,t){void 0===e&&(e={}),void 0===t&&(t={}),Object.keys(t).forEach(i=>{void 0===e[i]?e[i]=t[i]:X(t[i])&&X(e[i])&&Object.keys(t[i]).length>0&&U(e[i],t[i])})}// Прослуховувачі подій
...
for(;s;)t?s.matches(t)&&i.push(s):i.push(s),s=s.parentElement;return i}function ep(e,t,i){let s=J();return i?e["width"===t?"offsetWidth":"offsetHeight"]+parseFloat(s.getComputedStyle(e,null).getPropertyValue("width"===t?"margin-right":"margin-top"))+parseFloat(s.getComputedStyle(e,null).getPropertyValue("width"===t?"margin-left":"margin-bottom")):e.offsetWidth}function eu(){return e||(e=function(){let e=J(),t=Q();return{smoothScroll:t.documentElement&&t.documentElement.style&&"scrollBehavior"in t.documentElement.style,touch:!!("ontouchstart"in e||e.DocumentTouch&&t instanceof e.DocumentTouch)}}()),e}const ef=(e,t)=>{if(!e||e.destroyed||!e.params)return;let i=t.closest(e.isElement?"swiper-slide":`.${e.params.slideClass}`);if(i){let t=i.querySelector(`.${e.params.lazyPreloaderClass}`);!t&&e.isElement&&(t=i.shadowRoot.querySelector(`.${e.params.lazyPreloaderClass}`)),t&&t.remove()}},eh=(e,t)=>{if(!e.slides[t])return;let i=e.slides[t].querySelector('[loading="lazy"]');i&&i.removeAttribute("loading")},em=e=>{if(!e||e.destroyed||!e.params)return;let t=e.params.lazyPreloadPrevNext,i=e.slides.length;if(!i||!t||t<0)return;t=Math.min(t,i);let s="auto"===e.params.slidesPerView?e.slidesPerViewDynamic():Math.ceil(e.params.slidesPerView),n=e.activeIndex;if(e.params.grid&&e.params.grid.rows>1){let i=[n-t];i.push(...Array.from({length:t}).map((e,t)=>n+s+t)),e.slides.forEach((t,s)=>{i.includes(t.column)&&eh(e,s)});return}let a=n+s-1;if(e.params.rewind||e.params.loop)for(let s=n-t;s<=a+t;s+=1){let t=(s%i+i)%i;(t<n||t>a)&&eh(e,t)}else for(let s=Math.max(n-t,0);s<=Math.min(a+t,i-1);s+=1)s!==n&&(s>a||s<n)&&eh(e,s)};function eg(e){let{swiper:t,runCallbacks:i,direction:s,step:n}=e,{activeIndex:a,previousIndex:r}=t,o=s;if(o||(o=a>r?"next":a<r?"prev":"reset"),t.emit(`transition${n}`),i&&a!==r){if("reset"===o){t.emit(`slideResetTransition${n}`);return}t.emit(`slideChangeTransition${n}`),"next"===o?t.emit(`slideNextTransition${n}`):t.emit(`slidePrevTransition${n}`)}}function ev(e){let t=Q(),i=J(),s=this.touchEventsData;s.evCache.push(e);let{params:n,touches:a,enabled:r}=this;if(!r||!n.simulateTouch&&"mouse"===e.pointerType||this.animating&&n.preventInteractionOnTransition)return;!this.animating&&n.cssMode&&n.loop&&this.loopFix();let o=e;o.originalEvent&&(o=o.originalEvent);let l=o.target;if("wrapper"===n.touchEventsTarget&&!this.wrapperEl.contains(l)||"which"in o&&3===o.which||"button"in o&&o.button>0||s.isTouched&&s.isMoved)return;// change target el for shadow root component
...
o(t.wrapperEl,{attributes:!1})}}),s("destroy",()=>{a.forEach(e=>{e.disconnect()}),a.splice(0,a.length)})}]);var V=c("fAYBR");eI.use([function(e){let t,{swiper:i,extendParams:s,on:n,emit:a}=e,r="swiper-pagination";s({pagination:{el:null,bulletElement:"span",clickable:!1,hideOnClick:!1,renderBullet:null,renderProgressbar:null,renderFraction:null,renderCustom:null,progressbarOpposite:!1,type:"bullets",// 'bullets' or 'progressbar' or 'fraction' or 'custom'
 ...
 * _.isObject(null);
 * // => false
 */function eU(e){var t=typeof e;return!!e&&("object"==t||"function"==t)}/**
 ...
 * _.toNumber('3.2');
 * // => 3.2
 */function eK(e){if("number"==typeof e)return e;if("symbol"==typeof(t=e)||t&&"object"==typeof t&&"[object Symbol]"==eV.call(t))return eB;if(eU(e)){var t,i="function"==typeof e.valueOf?e.valueOf():e;e=eU(i)?i+"":i}if("string"!=typeof e)return 0===e?e:+e;e=e.replace(e$,"");var s=ej.test(e);return s||eF.test(e)?eG(e.slice(2),s?2:8):eD.test(e)?eB:+e}ez=/**
...
return void 0===l||i>=t||i<0||p&&s>=a}function m(){var e,i,s,n=eX();if(h(n))return g(n);// Restart the timer.
...
(o=void 0,u&&s)?f(e):(s=n=void 0,r))}function v(){var e,i=eX(),a=h(i);if(s=arguments,n=this,l=i,a){if(void 0===o)return(// Reset any `maxWait` timer.
c=e=l,// Start the timer for the trailing edge.
o=setTimeout(m,t),d?f(e):r);if(p)return(// Handle invocations in a tight loop.
o=setTimeout(m,t),f(l))}return void 0===o&&(o=setTimeout(m,t)),r}return t=eK(t)||0,eU(i)&&(d=!!i.leading,a=(p="maxWait"in i)?eR(eK(i.maxWait)||0,t):a,u="trailing"in i?!!i.trailing:u),v.cancel=function(){void 0!==o&&clearTimeout(o),c=0,s=l=n=o=void 0},v.flush=function(){return void 0===o?r:g(eX())},v};const eQ={form:document.querySelector(".js-form-search"),search:document.querySelector(".js-search"),time:document.querySelector(".js-time"),area:document.querySelector(".js-area"),ingredients:document.querySelector(".js-ingredients"),category:document.querySelector(".categories-js"),resetBtn:document.querySelector(".js-reset-filter-btn")};// ❗ Слухачі подій ❗
eQ.form.addEventListener("submit",function(e){e.preventDefault()}),eQ.time.addEventListener("change",function(e){W("time",e.currentTarget.value),$()}),eQ.area.addEventListener("change",function(e){W("area",e.currentTarget.value),$()}),eQ.ingredients.addEventListener("change",function(e){W("ingredient",e.currentTarget.value),$()}),eQ.search.addEventListener("input",/*@__PURE__*/r(ez)(()=>{W("title",eQ.search.value.trim()),$()},300)),// ❗ Створення відмальовки в DOM Area ❗
(0,u.searchAreas)().then(e=>{eQ.area.innerHTML=e.data.map(({name:e})=>`<option class="filter-opt" value="${e}">${e}</option>`).join("")}).catch(e=>{console.log(e)}),// ❗ Створення відмальовки в DOM Ingredients ❗
(0,u.searchIngredients)().then(e=>{eQ.ingredients.innerHTML=e.data.map(({name:e,_id:t})=>`<option class="filter-opt" value="${t}">${e}</option>`).join("")}).catch(e=>{}),// ❗ Створення Time ❗
function(){let e="";// ❗ ДОРОБИТИ ПРАВИЛЬНУ ЛОГІКУ ЧАСУ ❗
for(let t=0;t<220;t+=5)t%10==0&&(e+=`<option class="filter-opt" value="${t}">${t}</option>`);eQ.time.insertAdjacentHTML("beforeend",e)}(),// ❗ Створення js-reset-filter-btn ❗
eQ.resetBtn.addEventListener("click",function(e){q("time"),q("area"),q("ingredient"),q("title"),$()}//# sourceMappingURL=index.635a4e99.js.map
);
//# sourceMappingURL=index.635a4e99.js.map
