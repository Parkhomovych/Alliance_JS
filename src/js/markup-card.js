...
function markupModalRecipe(elem) {
  const { data: { title, youtube, preview, rating, time, ingredients, tags, instructions
  } } = elem;
  const tag = tags.map(tag => console.log(tag)).join('');
// Заміна шляху до відео youtube
  ...
  <iframe id="player" width="295" height="295"
  src="${changeHttpVideo}" style="border: none" allowfullscreen title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"></iframe>
