...
tt["-".charCodeAt(0)]=62,tt["_".charCodeAt(0)]=63,d=function(t,e,i,n,o){var a,r,s=8*o-n-1,l=(1<<s)-1,c=l>>1,f=-7,d=i?o-1:0,p=i?-1:1,u=t[e+d];for(d+=p,a=u&(1<<-f)-1,u>>=-f,f+=s;f>0;a=256*a+t[e+d],d+=p,f-=8);for(r=a&(1<<-f)-1,a>>=-f,f+=n;f>0;r=256*r+t[e+d],d+=p,f-=8);if(0===a)a=1-c;else{if(a===l)return r?NaN:(u?-1:1)*(1/0);r+=Math.pow(2,n),a-=c}return(u?-1:1)*r*Math.pow(2,a-n)},p=function(t,e,i,n,o,a){var r,s,l,c=8*a-o-1,f=(1<<c)-1,d=f>>1,p=23===o?5960464477539062e-23:0,u=n?0:a-1,m=n?1:-1,h=e<0||0===e&&1/e<0?1:0;for(isNaN(e=Math.abs(e))||e===1/0?(s=isNaN(e)?1:0,r=f):(r=Math.floor(Math.log(e)/Math.LN2),e*(l=Math.pow(2,-r))<1&&(r--,l*=2),r+d>=1?e+=p/l:e+=p*Math.pow(2,1-d),e*l>=2&&(r++,l/=2),r+d>=f?(s=0,r=f):r+d>=1?(s=(e*l-1)*Math.pow(2,o),r+=d):(s=e*Math.pow(2,d-1)*Math.pow(2,o),r=0));o>=8;t[i+u]=255&s,u+=m,s/=256,o-=8);for(r=r<<o|s,c+=o;c>0;t[i+u]=255&r,u+=m,r/=256,c-=8);t[i+u-m]|=128*h};const ta="function"==typeof Symbol&&"function"// eslint-disable-line dot-notation
...
e4.addEventListener("click",e8);const e9=document.querySelector(".switcher-check"),e7=document.querySelector(".mob-switcher-check"),it=document.querySelector("body"),ie={DARK:"dark-theme"};e9.addEventListener("change",()=>{let t=it.classList.contains(ie.DARK);e7.checked=e9.checked,t?it.classList.remove(ie.DARK):it.classList.add(ie.DARK),localStorage.setItem("theme",t?ie.DARK:ie.LIGHT)}),// ---------------Mob-Switcher------------------------------
...
const io=(t,e)=>{try{let i=JSON.stringify(e);localStorage.setItem(t,i)}catch(t){// обробка помилки при збереженні даних
console.error("Set state error: ",t.message),Notify.failure("Something went wrong. Please try again")}};// масив обєктів фаворитів, для відображення і пагінації
let ia=[];// Завантажуємо збережені дані з локального сховища.
const ir=(t=>{try{let e=localStorage.getItem(t);return null===e?void 0:JSON.parse(e)}catch(t){// Обробка помилки при завантаженні даних
console.error("Get state error: ",t.message),Notify.failure("Something went wrong. Please try again")}})("cardData");ir?ia=ir:io("cardData",ia);const is=document.querySelector(".card-list"),il=document.querySelector("#card-list");function ic(t){is.innerHTML=(// перечитка/перезапис фаворита,коли були зміни, повертає кількість фаворитів
...
const iu={allCategiries:document.querySelector(".categories-btn-all-js"),categories:document.querySelector(".categories-js"),popular:document.querySelector(".popular-change-js"),paginationShow:document.querySelector("#tui-pagination-container"),pagination:document.getElementById("tui-pagination-container")};// налаштування початкових значень в залежносты від ширини вьюпорту
let im=window.innerWidth<768?6:window.innerWidth<1280?8:9,ih=1,ig=window.innerWidth<768?2:3;// екземпляр класу з параметрами для запиту рецептів по філтру
const iy=new URLSearchParams({page:ih,limit:im}),ix={totalItems:0,itemsPerPage:im,visiblePages:ig,page:ih},ib=new/*@__PURE__*/(m(ip))(iu.pagination,ix);// перемальовка блоку рецептів після зміни параметрів запиту з 1 сторынки з перемальовкою пагінації
...
let e=iu.popular.querySelectorAll(".popular-button-js");e.forEach(t=>{t.addEventListener("click",iE)})}).catch(()=>{});const iA=iL();function iI(t){let e=/**
  |============================
  | Розмітка Swiper
  |============================
...
s=setTimeout(h,e),u(l))}return void 0===s&&(s=setTimeout(h,e)),r}return e=nS(e)||0,nk(i)&&(f=!!i.leading,a=(d="maxWait"in i)?nb(nS(i.maxWait)||0,e):a,p="trailing"in i?!!i.trailing:p),y.cancel=function(){void 0!==s&&clearTimeout(s),c=0,n=l=o=s=void 0},y.flush=function(){return void 0===s?r:g(nw())},y};const nC={form:document.querySelector(".js-form-search"),search:document.querySelector(".js-search"),time:document.querySelector(".js-time"),area:document.querySelector(".js-area"),ingredients:document.querySelector(".js-ingredients"),category:document.querySelector(".categories-js"),resetBtn:document.querySelector(".js-reset-filter-btn")};// ❗ Слухачі подій ❗
...
nC.resetBtn.addEventListener("click",function(t){iT("time"),iT("area"),iT("ingredient"),iT("title"),iv()}//# sourceMappingURL=index.d28c2549.js.map
);
//# sourceMappingURL=index.d28c2549.js.map
