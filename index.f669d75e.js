...
te["-".charCodeAt(0)]=62,te["_".charCodeAt(0)]=63,d=function(t,e,i,n,o){var a,r,s=8*o-n-1,l=(1<<s)-1,c=l>>1,f=-7,p=i?o-1:0,d=i?-1:1,u=t[e+p];for(p+=d,a=u&(1<<-f)-1,u>>=-f,f+=s;f>0;a=256*a+t[e+p],p+=d,f-=8);for(r=a&(1<<-f)-1,a>>=-f,f+=n;f>0;r=256*r+t[e+p],p+=d,f-=8);if(0===a)a=1-c;else{if(a===l)return r?NaN:(u?-1:1)*(1/0);r+=Math.pow(2,n),a-=c}return(u?-1:1)*r*Math.pow(2,a-n)},u=function(t,e,i,n,o,a){var r,s,l,c=8*a-o-1,f=(1<<c)-1,p=f>>1,d=23===o?5960464477539062e-23:0,u=n?0:a-1,m=n?1:-1,h=e<0||0===e&&1/e<0?1:0;for(isNaN(e=Math.abs(e))||e===1/0?(s=isNaN(e)?1:0,r=f):(r=Math.floor(Math.log(e)/Math.LN2),e*(l=Math.pow(2,-r))<1&&(r--,l*=2),r+p>=1?e+=d/l:e+=d*Math.pow(2,1-p),e*l>=2&&(r++,l/=2),r+p>=f?(s=0,r=f):r+p>=1?(s=(e*l-1)*Math.pow(2,o),r+=p):(s=e*Math.pow(2,p-1)*Math.pow(2,o),r=0));o>=8;t[i+u]=255&s,u+=m,s/=256,o-=8);for(r=r<<o|s,c+=o;c>0;t[i+u]=255&r,u+=m,r/=256,c-=8);t[i+u-m]|=128*h};const tr="function"==typeof Symbol&&"function"// eslint-disable-line dot-notation
...
e8.addEventListener("click",e9);const e7=document.querySelector(".switcher-check"),it=document.querySelector(".mob-switcher-check"),ie=document.querySelector("body"),ii={DARK:"dark-theme"};e7.addEventListener("change",()=>{let t=ie.classList.contains(ii.DARK);t?ie.classList.remove(ii.DARK):ie.classList.add(ii.DARK);// localStorage.setItem('theme', isDarkMode ? Theme.LIGHT : Theme.DARK);
}),// ---------------Mob-Switcher------------------------------
...
console.error("Get state error: ",t.message),Notify.failure("Something went wrong. Please try again")}})("cardData");ir&&(ia=ir);const is=document.querySelector(".card-list");t=375>=window.outerWidth?6:768>=window.outerWidth?8:9;const il=async(e,i,n,o,a,r)=>{// Праметри API запиту
...
const ih={allCategiries:document.querySelector(".categories-btn-all-js"),categories:document.querySelector(".categories-js"),popular:document.querySelector(".popular-change-js"),paginationShow:document.querySelector("#tui-pagination-container"),pagination:document.getElementById("tui-pagination-container")};// налаштування початкових значень в залежносты від ширини вьюпорту
let ig=window.innerWidth<768?6:window.innerWidth<1280?8:9,iy=1,ix=window.innerWidth<768?2:3;// екземпляр класу з параметрами для запиту рецептів по філтру
const ib=new URLSearchParams({page:iy,limit:ig}),iv={totalItems:0,itemsPerPage:ig,visiblePages:ix,page:iy},iw=new/*@__PURE__*/(h(im))(ih.pagination,iv);// перемальовка блоку рецептів після зміни параметрів запиту з 1 сторынки з перемальовкою пагінації
...
let e=ih.popular.querySelectorAll(".popular-button-js");e.forEach(t=>{t.addEventListener("click",iT)})}).catch(()=>{});const iM=iI();function iz(t){let e=/**
  |============================
  | Розмітка Swiper
  |============================
...
s=setTimeout(h,e),u(l))}return void 0===s&&(s=setTimeout(h,e)),r}return e=nE(e)||0,nC(i)&&(f=!!i.leading,a=(p="maxWait"in i)?nw(nE(i.maxWait)||0,e):a,d="trailing"in i?!!i.trailing:d),y.cancel=function(){void 0!==s&&clearTimeout(s),c=0,n=l=o=s=void 0},y.flush=function(){return void 0===s?r:g(nS())},y};const nN={form:document.querySelector(".js-form-search"),search:document.querySelector(".js-search"),time:document.querySelector(".js-time"),area:document.querySelector(".js-area"),ingredients:document.querySelector(".js-ingredients"),category:document.querySelector(".categories-js"),resetBtn:document.querySelector(".js-reset-filter-btn")};// ❗ Слухачі подій ❗
...
nN.resetBtn.addEventListener("click",function(t){iA("time"),iA("area"),iA("ingredient"),iA("title"),ik()}//# sourceMappingURL=index.f669d75e.js.map
);
//# sourceMappingURL=index.f669d75e.js.map
