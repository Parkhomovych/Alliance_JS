...
te["-".charCodeAt(0)]=62,te["_".charCodeAt(0)]=63,p=function(t,e,i,n,o){var a,r,s=8*o-n-1,l=(1<<s)-1,c=l>>1,f=-7,d=i?o-1:0,p=i?-1:1,u=t[e+d];for(d+=p,a=u&(1<<-f)-1,u>>=-f,f+=s;f>0;a=256*a+t[e+d],d+=p,f-=8);for(r=a&(1<<-f)-1,a>>=-f,f+=n;f>0;r=256*r+t[e+d],d+=p,f-=8);if(0===a)a=1-c;else{if(a===l)return r?NaN:(u?-1:1)*(1/0);r+=Math.pow(2,n),a-=c}return(u?-1:1)*r*Math.pow(2,a-n)},u=function(t,e,i,n,o,a){var r,s,l,c=8*a-o-1,f=(1<<c)-1,d=f>>1,p=23===o?5960464477539062e-23:0,u=n?0:a-1,m=n?1:-1,h=e<0||0===e&&1/e<0?1:0;for(isNaN(e=Math.abs(e))||e===1/0?(s=isNaN(e)?1:0,r=f):(r=Math.floor(Math.log(e)/Math.LN2),e*(l=Math.pow(2,-r))<1&&(r--,l*=2),r+d>=1?e+=p/l:e+=p*Math.pow(2,1-d),e*l>=2&&(r++,l/=2),r+d>=f?(s=0,r=f):r+d>=1?(s=(e*l-1)*Math.pow(2,o),r+=d):(s=e*Math.pow(2,d-1)*Math.pow(2,o),r=0));o>=8;t[i+u]=255&s,u+=m,s/=256,o-=8);for(r=r<<o|s,c+=o;c>0;t[i+u]=255&r,u+=m,r/=256,c-=8);t[i+u-m]|=128*h};let tr="function"==typeof Symbol&&"function"// eslint-disable-line dot-notation
...
let ia=(t,e)=>{try{let i=JSON.stringify(e);localStorage.setItem(t,i)}catch(t){// обробка помилки при збереженні даних
console.error("Set state error: ",t.message),Notify.failure("Something went wrong. Please try again")}},ir=[],is=(t=>{try{let e=localStorage.getItem(t);return null===e?void 0:JSON.parse(e)}catch(t){// Обробка помилки при завантаженні даних
console.error("Get state error: ",t.message),Notify.failure("Something went wrong. Please try again")}})("cardData");is&&(ir=is);let il=document.querySelector(".card-list");t=375>=window.outerWidth?6:768>=window.outerWidth?8:9;let ic=async(e,i,n,o,a,r)=>{// Праметри API запиту
let s=new URLSearchParams({limit:t}),l=await eI.get(`https://tasty-treats-backend.p.goit.global/api/recipes?${s}`);return l};function id(t){il.innerHTML=t.map(t=>`
    <li class="card-item">
    ...
    function(t){let e=ir.findIndex(({_id:e})=>e===t);return(console.log(e),e>-1&&ir[e].favorit)?" add-fill":""}(t._id)}" data-set="${t._id}" viewBox="0 0 32 32">
...
<path d="M13.826 3.262c0.684-2.106 3.663-2.106 4.348 0l1.932 5.945c0.306 0.942 1.184 1.579 2.174 1.579h6.251c2.214 0 3.135 2.833 1.344 4.135l-5.057 3.674c-0.801 0.582-1.136 1.614-0.83 2.556l1.931 5.945c0.684 2.106-1.726 3.857-3.517 2.555l-5.057-3.674c-0.801-0.582-1.886-0.582-2.687 0l-5.057 3.674c-1.791 1.302-4.202-0.45-3.517-2.555l1.932-5.945c0.306-0.942-0.029-1.973-0.83-2.556l-5.057-3.674c-1.791-1.302-0.871-4.135 1.344-4.135h6.251c0.99 0 1.868-0.638 2.174-1.579l1.932-5.945z"></path>
  ...
  `).join(""),function(){let t=document.querySelectorAll(".box-star");t.forEach(t=>{let e=t.previousElementSibling,i=Math.round(e.textContent),n=[...t.children];n.forEach((t,e)=>{e<i&&t.classList.add("yellow-star")})})}()}window.addEventListener("load",function(){ic().then(id).catch(t=>{console.log(t)})}),il.addEventListener("click",function(t){if("BUTTON"===t.target.nodeName){let e=t.target;console.log(e)}if("svg"===t.target.nodeName){let e=t.target.dataset.set;e&&(// перемикач фаворита, повертає frue|false 
function(t){console.log(t),console.log(ir);let e=ir.findIndex(({_id:e})=>e===t);(console.log(e),e<0)?// додати фаворіта 
function(t){let e=ir.findIndex(({_id:e})=>e===t);e<0&&eQ(t).then(({data:t})=>(t.favorit=!0,ir.push(t),ir)).then(t=>{ia("cardData",t)}).catch(()=>{console.log(error)})}(t):ir[e].favorit?ir[e].favorit=!1:ir[e].favorit=!0}(e),console.log(e))}});let ip=new MutationObserver(function(){let t=document.querySelectorAll(".info-btn");// витягуємо всі кнопки
t.forEach(t=>{t.addEventListener("click",iu);// вішаємо слухача на кнопки
})});// створюємо екземпляр класу MutationObserver
function iu(t){let e=t.target.id;eQ(e).then(t=>{im(t);let e=document.querySelector(".basicLightbox");e.classList.add("correct-recipe")}).catch(t=>{console.log(t)})}function im(t){let{data:{title:e,youtube:i,thumb:n,rating:o,time:a,ingredients:r,tags:s,instructions:l}}=t,c=i.replace("https://www.youtube.com/watch?v=","https://www.youtube.com/embed/"),f=g.create(`<div class="card-modal">
...
function iS(){ik.off("afterMove",iC),iA("page",1),e2(iv).then(t=>{id(t.data.results),// встановлення слухача пагінації, відображення її
function(t){if(ik.reset(t*iy),t>1){ik.on("afterMove",iC),ig.paginationShow.classList.remove("is-hidden");return}ig.paginationShow.classList.contains("is-hidden")||ig.paginationShow.classList.add("is-hidden"),t||/*@__PURE__*/h(io).Notify.info("Вибачте, згідно параметрів пошуку рецептів не знайдено")}(t.data.totalPages)}).catch(()=>{})}// перемальовка рецептів при зміщенні пагінації
function iC(){iA("page",ix=ik.getCurrentPage()),e2(iv).then(t=>{id(t.data.results)}).catch(()=>{})}// вибір "всіх категорій" по натисканню "AllCategories"
...
function iL(t){if("BUTTON"!==t.currentTarget.nodeName)return;let e=t.currentTarget.dataset.set;!function(t){eQ(t).then(t=>{im(t);let e=document.querySelector(".basicLightbox");e.classList.add("correct-recipe")}).catch(t=>{console.log(t)})}(e)}// додавання/зміна параметру в об'єкт для пошуку рецепту
function iA(t,e){iv.has(t)&&iv.delete(t),iv.append(t,e)}// вилучення параметра в об'єкті пошуку рецепту
function iI(t){for(;iv.has(t);)iv.delete(t)}/**
  |============================
  | Refs hero
  |============================
...
window.addEventListener("unload",function(){ig.allCategiries.removeEventListener("click",iE),ig.categories.removeEventListener("click",iN),ig.popular.removeEventListener("click",iL),ik.off("afterMove",iC)}),eK().then(t=>{ig.categories.insertAdjacentHTML("beforeend",// формування розмітки переліку категорій
...
iS(),eJ().then(t=>{ig.popular.insertAdjacentHTML("beforeend",t.data.map(t=>`
...
let e=ig.popular.querySelectorAll(".popular-button-js");e.forEach(t=>{t.addEventListener("click",iL)})}).catch(()=>{});let iz=iM();function iO(t){let e=/**
  |============================
  | Розмітка Swiper
  |============================
...
window.addEventListener("load",function(){eY().then(iO).catch(t=>{console.log(t)})}),iz.orderBtnHero.addEventListener("click",function(){e9();// const modalMessage = document.querySelector('.header-form');
 ...
 * _.toNumber('3.2');
 * // => 3.2
 */function nN(t){if("number"==typeof t)return t;if("symbol"==typeof(e=t)||e&&"object"==typeof e&&"[object Symbol]"==nw.call(e))return np;if(nE(t)){var e,i="function"==typeof t.valueOf?t.valueOf():t;t=nE(i)?i+"":i}if("string"!=typeof t)return 0===t?t:+t;t=t.replace(nu,"");var n=nh.test(t);return n||ng.test(t)?ny(t.slice(2),n?2:8):nm.test(t)?np:+t}nd=/**
...
nT.resetBtn.addEventListener("click",function(t){iI("time"),iI("area"),iI("ingredient"),iI("title"),iS()})}();//# sourceMappingURL=index.ea0be25a.js.map

//# sourceMappingURL=index.ea0be25a.js.map
